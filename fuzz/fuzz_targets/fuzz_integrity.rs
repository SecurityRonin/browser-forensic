...
fuzz_target!(|data: &[u8]| {
    ...
    match selector % 4 {
        0 => {
            let _ = browser_forensic_integrity::check_database_integrity(path);
        }
        1 => {
            let _ = browser_forensic_integrity::check_wal_state(path);
        }
        2 => {
            let _ = browser_forensic_integrity::check_history_integrity(path, BrowserFamily::Chromium);
        }
        _ => {
            let _ = browser_forensic_integrity::check_cookie_integrity(path, BrowserFamily::Chromium);
        }
    }
});
