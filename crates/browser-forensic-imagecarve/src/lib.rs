...
//! memory image — not just a single database's freelist/WAL.
//!
//! This crate carves over a [`forensic_vfs::ImageSource`] and nothing else. It
//! does NOT open evidence containers: per ADR-0016 a PARSER-layer crate depends
//! on FOUNDATION only and takes bytes, never a CONTAINER or ORCHESTRATION crate.
//! Opening an image through the container abstraction — which decodes raw/dd,
//! E01/EWF, VMDK, VHDX, VHD, QCOW2, DMG and ISO9660 behind one interface, so a
//! compressed E01 carves its real contents rather than compressed bytes — is
//! wiring, and lives in the layer allowed to do it (`browser-forensic-cli`).
//!
//! Given such a source, this crate streams it in bounded, overlapping windows,
