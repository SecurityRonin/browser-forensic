...
fn triage_chromium_profile_with_history_and_cookies() {
    ...
    assert!(
        report
            .events
            .iter()
            .any(|e| e.artifact == browser_forensic_core::ArtifactKind::Cookies),
        "should have cookie events from Cookies database"
    );

    // Integrity: visit ID gap should be detected (visits 1 -> 10)
    assert!(
        report.integrity.iter().any(|i| matches!(
            i,
            browser_forensic_integrity::IntegrityIndicator::VisitIdGap { .. }
        )),
