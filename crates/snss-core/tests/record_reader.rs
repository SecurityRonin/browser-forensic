...
fn session_histogram_matches_ground_truth() {
    ...
    assert!(stream.warnings.is_empty(), "clean file, no truncation: {:?}", stream.warnings);

    let h = histogram(&stream);
    let expected: &[(u8, usize)] = &[
        (0, 1619), (2, 3161), (6, 1826), (7, 1619), (8, 2), (9, 1),
        (12, 1545), (13, 9), (14, 1), (19, 1619), (21, 1620), (23, 2),
        (25, 1542), (32, 2), (255, 1),
    ];
...
fn apps_histogram_matches_ground_truth() {
    ...
    let h = histogram(&stream);
    let expected: &[(u8, usize)] = &[
        (0, 1), (6, 4), (7, 5), (8, 2), (9, 1), (12, 1), (14, 1),
        (15, 1), (16, 1), (17, 1), (19, 1), (20, 1), (21, 1), (23, 3),
        (32, 2), (255, 1),
    ];
    assert_eq!(h, expected.iter().copied().collect());
}
...
fn only_the_sentinel_has_an_empty_payload() {
    let Some(stream) = open_fixture_or_skip("Tabs_real") else { return };
    let empties: Vec<u8> =
        stream.records.iter().filter(|r| r.payload.is_empty()).map(|r| r.id).collect();
    assert_eq!(empties, vec![255], "only the cmd-255 sentinel is payload-less");

    // Navigation commands in the Tabs dialect (id 1) always carry URL/title bytes.
    assert!(stream.records.iter().filter(|r| r.id == 1).all(|r| !r.payload.is_empty()));
}
