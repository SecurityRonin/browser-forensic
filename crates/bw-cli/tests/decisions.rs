...
//! Humble-Object extraction: the pure decisions pulled out of the `bw` `run_*`
//! glue — carve-stat merging, the triage text summary, and filename-based browser
//! inference. `main`'s `run_*` now build these and only `println!` the result, so
//! the print loops are the irreducible shell.

use browser_carve::CarveStats;
use bw_cli::{infer_browser_from_filename, merge_carve_stats, triage_summary_lines};
use std::path::Path;
...
fn infer_browser_safari_history_db() {
    assert_eq!(
        ...
        Some(browser_core::BrowserFamily::Safari)
    );
}
...
fn infer_browser_firefox_artifacts() {
    for name in [
        ...
        assert_eq!(
            ...
            Some(browser_core::BrowserFamily::Firefox),
